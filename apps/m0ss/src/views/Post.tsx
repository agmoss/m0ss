...
const TagChip = styled(Chip)(({ theme, color }) => ({
  ...
  backgroundColor:
    color === "primary" ? theme.palette.primary.main : color === "info" ? theme.palette.info.main : undefined,
  color: color === "primary" || color === "info" ? theme.palette.common.white : undefined,
}))
...
export const Post = ({ str }: IStr) => {
  ...
  const getMetadata = () => {
    ...
    if (str.toLowerCase().includes("typescript") || str.toLowerCase().includes("ts")) tags.push("TypeScript")
    if (str.toLowerCase().includes("javascript") || str.toLowerCase().includes("js")) tags.push("JavaScript")
    if (str.toLowerCase().includes("graphql")) tags.push("GraphQL")
    if (str.toLowerCase().includes("api")) tags.push("API")
    if (str.toLowerCase().includes("nextjs") || str.toLowerCase().includes("next.js")) tags.push("Next.js")
  ...
  return (
    <Root>
      <Container maxWidth="md" sx={{ py: 8 }}>
        <Button startIcon={<ArrowBack />} onClick={() => router.push(Paths.POSTS)} sx={{ mb: 4 }}>
          Back to Posts
        </Button>
            ...
            <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1, mb: 3 }}>
              {metadata.tags.map((tag) => (
                <TagChip
                  ...
                  label={tag}
                  size="small"
                  color={tag === "React" ? "info" : tag === "TypeScript" ? "primary" : "default"}
                />
              ))}
            </Box>
          )}
        ...
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <Button startIcon={<ArrowBack />} onClick={() => router.push(Paths.POSTS)}>
            Back to Posts
          </Button>
